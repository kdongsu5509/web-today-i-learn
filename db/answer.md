...
- `crew_id` 와 `nickname` 이 중복되는 컬럼이다.
...
```sql
CREATE TABLE crew (id INT PRIMARY KEY, nickname VARCHAR(255) NOT NULL);
```
...
```sql
ALTER TABLE attendance
ADD CONSTRAINT uq_crew_attendance_date
...
```sql
SELECT c.nickname, a.start_time
FROM crew c
JOIN attendance a ON c.id = a.crew_id
WHERE c.nickname LIKE '디%' 
  AND a.attendance_date = '2025-03-04';
```
...
```sql
SELECT c.id, c.nickname, a.attendance_date, a.start_time, a.end_time
FROM crew c
LEFT JOIN attendance a ON c.id = a.crew_id AND a.attendance_date = '2025-03-06'
WHERE c.nickname = '어셔';
```
...
WHERE attendance_id = 14 AND attendance_date = '2025-03-12';
```
...
```sql
SELECT c.nickname, COUNT(a.start_time) AS attendance_count
FROM attendance a
JOIN crew c ON a.crew_id = c.id
